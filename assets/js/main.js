document.addEventListener('DOMContentLoaded', () => {
	...
	if (mpesaForm) {
		mpesaForm.addEventListener('submit', async (e) => {
			...
			const submitBtn = mpesaForm.querySelector('[type="submit"]');
			submitBtn.disabled = true;
			mpesaMessage.textContent = 'Connecting to M-Pesa…';
			mpesaMessage.className = 'alert alert-info d-block';

			try {
				// ── 1. Fetch a CSRF token (required by requireCsrf() middleware) ──
				// Build the API base path relative to this page's location so it
				// works both on localhost subdirectories and on the production domain.
				const apiBase = window.location.pathname
					.replace(/\/[^/]+\.php$/, '')   // strip the PHP filename
					.replace(/\/$/, '')             // strip trailing slash
					+ '/api';

				console.log('[MPESA] API Base URL resolved to:', apiBase);
				const csrfRes = await fetch(`${apiBase}/auth/csrf`, {
					credentials: 'include',
				});
				if (!csrfRes.ok) throw new Error('Could not retrieve CSRF token.');
				const { csrf_token } = await csrfRes.json();

				// ── 2. Submit STK Push request ──────────────────────────────────
				...
				const response = await fetch(`${apiBase}/payments/initiate`, {
					method: 'POST',
					credentials: 'include',
					headers: {
				...
				if (response.ok && result.message) {
					mpesaMessage.textContent = '✔ ' + result.message;
				...
				mpesaMessage.textContent = 'Network error. Please check your connection and try again.';
				mpesaMessage.className = 'alert alert-danger d-block';
			} finally {
